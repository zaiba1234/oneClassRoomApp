...
const ProfileScreen = ({ navigation }) => {
  ...
  // Debug user data from Redux
  console.log('🔍 ProfileScreen: Redux user data:', {
    fullName,
    mobileNumber,
    _id,
    userId,
  ...
  console.log('🔍 ProfileScreen: Email field analysis:', {
    'email': email,
    'typeof email': typeof email,
    'email === ""': email === '',
    'email === null': email === null,
    'email === undefined': email === undefined,
    'email length': email ? email.length : 'N/A',
    'email truthy': !!email
  });
  ...
  // Function to ensure user profile exists before deletion
  const ensureUserProfileExists = async () => {
    try {
      console.log('🔍 Ensuring user profile exists...');
      
      const apiUrl = getApiUrl('/api/user/profile/update-profile');
      const requestHeaders = {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
      };

      // Make a minimal update to ensure profile exists
      ...
      if (response.ok) {
        ...
        console.log('⚠️ Profile creation failed:', errorData);
        return false;
      }
    } catch (error) {
  ...
  const deleteAccountAPI = async () => {
    try {
      ...
      const profileEnsured = await ensureUserProfileExists();
      
      if (!profileEnsured) {
        console.log('⚠️ Could not ensure profile exists, proceeding with deletion anyway...');
      }
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        console.error('❌ Delete account failed - Full error response:', JSON.stringify(errorData, null, 2));
